...
DEFAULT_EXCEL = r"E:\Siraj\assam_crv\file\Mitigation intervention master table for sw team v1.xlsx"
...
def get_db_config(args):
    ...
    return {
        "dbname": args.dbname or env.get("DB_NAME") or DEFAULT_DB,
        "user": args.user or env.get("DB_USER") or "postgres",
        "password": args.password or env.get("DB_PASSWORD") or "",
        "host": args.host or env.get("DB_HOST") or "localhost",
        "port": int(args.port or env.get("DB_PORT") or 5432),
    }
...
def import_data(conn, source_path, sheet_name, table, truncate):
    ...
    header_aliases = {
        "Theme": ["Theme", "theme"],
        "Sub theme": ["Sub theme", "subtheme"],
        "Mitigation intervention": ["Mitigation intervention", "intervention_name"],
        "Vulnerable asset": ["Vulnerable asset", "vulnerable_asset"],
        "Intervention type": ["Intervention type", "intervention_type"],
        "Display note": [
        ...
        "Unit cost (Rs)": ["Unit cost (Rs)", "unit_cost_rs"],
    }

    def find_header(key):
        for candidate in header_aliases.get(key, []):
    ...
    with conn.cursor() as cur:
        ...
        for row in rows:
            theme = clean_text(get_cell(row, find_header("Theme")))
            subtheme = clean_text(get_cell(row, find_header("Sub theme")))
            intervention_name = clean_text(get_cell(row, find_header("Mitigation intervention")))
            ...
            data = (
                theme,
                subtheme,
                clean_text(get_cell(row, find_header("Vulnerable asset"))),
                clean_text(get_cell(row, find_header("Intervention type"))),
                intervention_name,
                clean_text(
                    get_cell(
                        row,
                        find_header("Display note"),
                    )
                ),
                clean_text(get_cell(row, find_header("Unit"))),
                clean_decimal(get_cell(row, find_header("Quantity"))),
                clean_decimal(get_cell(row, find_header("Unit cost (Rs)"))),
                "active",
            )
