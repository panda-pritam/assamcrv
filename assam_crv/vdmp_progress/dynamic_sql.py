...
def get_dynamic_sql_script(village_id, model_name='HouseholdSurvey'):
    ...
    # Get active mappings for the model
    mappings = AttributeMapping.objects.filter(
        model_name=model_name,
        is_active=True
    ).values('mobile_db_attribute_id', 'attribute_text', 'alias_name')
    
    # Build dynamic SELECT clauses
    select_clauses = []
    for mapping in mappings:
        if mapping['mobile_db_attribute_id']:
            if mapping['attribute_text']:
                clause = f"MAX(CASE WHEN a.attribute_name ILIKE '%%{mapping['attribute_text']}%%' THEN av.value END) AS {mapping['alias_name']}"
            else:
                clause = f"MAX(CASE WHEN a.id = {mapping['mobile_db_attribute_id']} THEN av.value END) AS {mapping['alias_name']}"
