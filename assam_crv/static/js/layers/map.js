    ...
    totalLayers > 1
      ? `
        <div class="popup-navigation d-flex justify-content-between align-items-center mb-1 p-2 bg-light rounded">
            <button id="prevLayerBtn" class="btn btn-sm btn-outline-primary ${
            ...
            <button id="nextLayerBtn" class="btn btn-sm btn-outline-primary ${
...
function navigateLayer(direction) {
  ...
  if (newIndex >= 0 && newIndex < clickedLayers.length) {
    ...
    updatePopupContent();
    console.log(
      `Navigated to layer ${currentLayerIndex + 1} of ${
        clickedLayers.length
      }: ${clickedLayers[currentLayerIndex].name}`
    );
  }
}
...
function getCurrentCQLFilter() {
  ...
  const districtId =
    districtSelect?.selectedOptions[0]?.getAttribute("code") || null;
  const villageId =
    villageSelect?.selectedOptions[0]?.getAttribute("code") || null;
...
function zoomToLatLon(lat, lon, zoomLevel = 12) {
  ...
  console.log(
    `Zoomed to coordinates: ${lat}, ${lon} at zoom level ${zoomLevel}`
  );
}
