...
function renderVDMPChart(data) {
  ...
  // Update chart title
  document.getElementById("chartTitle").textContent = gettext(
    "VDMP Activity Progress Status"
  );
  ...
  const options = {
    ...
    chart: {
      type: "bar",
      // ✅ Use 100% to fill container height
      height: '100%',
      stacked: true,
      toolbar: {
        show: true,
  ...
  chartInstance
    ...
    .catch(error => {
      console.error("Error rendering chart:", error);
      chartDiv.innerHTML =
        '<div class="text-center p-5 text-danger">Error rendering chart</div>';
    });
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!chartDiv) {
    console.error(
      "Chart container not found! Check the ID: VDMP_progress_page_chart"
    );
  } else {
...
function getSelectedActivityIds() {
  ...
  return $(".activity-checkbox:checked")
    .map(function () {
...
function initializeDetailedProgressTable() {
  ...
  detailedProgressTable = initializeDataTable(
    "vdmp_detailed_progress_table",
    columns
  );
}

// Original function - loads both chart and table (used for initial load)
async function getTableAndChartData(
  district_id = "",
  circle_id = "",
  gram_panchayat_id = "",
  village_id = "",
  ...
  try {
    ...
    if (gram_panchayat_id)
      params.append("gram_panchayat_id", gram_panchayat_id);
    if (village_id) params.append("village_id", village_id);
    if (activity_ids && activity_ids.length > 0)
      params.append("activity_ids", activity_ids.join(","));
    ...
    document.getElementById(
      "VDMP_progress_page_chart"
    ).innerHTML = `<div class="text-center p-5 text-danger">Error loading chart data: ${error.message}</div>`;
    hideVDMPChartLoader();
    hideVDMPSummaryTableLoader();
  }
}
...
function renderVDMPTable(data) {
  ...
  data.forEach((item, index) => {
    const total =
      parseInt(item.not_started) +
      parseInt(item.in_progress) +
...
function hideVDMPChartLoader() {
  const chartDiv = document.getElementById("VDMP_progress_page_chart");
  if (chartDiv) {
    const loaderElements = chartDiv.querySelectorAll(
      ".spinner-border, .d-flex"
    );
