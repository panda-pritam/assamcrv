...
function initializeCharts() {
  ...
  const themeData = [
    {
      series: [40, 35, 25],
      labels: ["Quality Education", "food security", "health facilities"],
      colors: ["#34495e", "#e67e22", "#95a5a6"],
    },
    {
      series: [28, 42, 30],
      labels: ["Knowledge skills", "Income + livelihood", "market access"],
      colors: ["#8e44ad", "#d35400", "#27ae60"],
    },
    {
      series: [38, 32, 30],
      labels: ["Development of VDMP", "Community camps", "local governance"],
      colors: ["#2980b9", "#c0392b", "#16a085"],
    },
    {
      series: [33, 37, 30],
      labels: ["Access to EWS", "Village awareness", "training programs"],
      colors: ["#7f8c8d", "#f1c40f", "#e91e63"],
    },
  ];
...
async function renderResilienceCharts(data) {
  try {
    ...
    chartConfigs.forEach(config => {
      ...
      const resilient = categories.map(
        key => data[config.dataKey][key].resiliant
      );
      const noResilient = categories.map(
        key => data[config.dataKey][key].no_resiliant
      );
      ...
      const options = {
        ...
        colors: [getRandomColor(res_colors), getRandomColor(no_colors)],
        legend: {position: "bottom"},
